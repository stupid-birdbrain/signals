...
public partial class Components {
    ...
    internal static uint ComponentCount;
    internal static uint ComponentMasksPerEntity = 1;

    internal static Info[] ComponentInfos = Array.Empty<Info>();

    private static readonly Dictionary<Type, Handle> component_by_type = new();

    private static readonly Dictionary<string, Handle> component_name_to_handle = new();
    private static readonly Dictionary<Handle, Type> component_handle_to_type = new();

    private static Handle RegisterComponentType<T>() where T : struct, IComponent {
        lock(component_by_type) {
            if(component_by_type.TryGetValue(typeof(T), out var existingHandle)) {
                return existingHandle;
            }

            var handle = new Handle(++ComponentCount);
            if(handle.Id >= ComponentInfos.Length) {
            ...
            ComponentInfos[handle.Id] = new Info
            {
                ...
                HasWorldComponentFunc = HasWorldComponent<T>
            };

            component_by_type[typeof(T)] = handle;
            component_name_to_handle[typeof(T).Name] = handle;
            component_handle_to_type[handle] = typeof(T);

            uint newMasksPerEntityNeeded = (handle.Id / Bitset256.CAPACITY) + 1;
    ...
    internal static Handle GetComponentHandle(Type componentType) {
        lock(component_by_type) {
            if(component_by_type.TryGetValue(componentType, out var handle)) {
                return handle;
            }
    ...
    internal static bool TryGetComponentHandleFromName(string componentName, out Handle handle) => component_name_to_handle.TryGetValue(componentName, out handle);

    internal static Type GetComponentType(Handle handle) {
        if(component_handle_to_type.TryGetValue(handle, out var type)) {
            return type;
        }
    ...
    internal static void AddComponentByHandle(Entity entity, Handle handle, object value) {
        ...
        var componentInfo = ComponentInfos[handle.Id];
